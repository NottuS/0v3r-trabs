...
/*    ===   INICIA MODIFICAÇÃO   ===   */
      ...
      NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds () << "s client " <<
                   ipv4->GetAddress (ipv4->GetInterfaceForDevice (m_socket->GetNode()->GetDevice(1)), 0).GetLocal() <<
...
/*    ===   INICIA MODIFICAÇÃO   ===   */
          ...
          NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds () << "s client " <<
                       ipv4->GetAddress (ipv4->GetInterfaceForDevice (socket->GetNode()->GetDevice(1)), 0).GetLocal() <<
