...
/*    ===   INICIA MODIFICAÇÃO   ===   */
void Ipv4InterfaceContainer::SetRouter (uint32_t i, bool router)
{
  Ptr<Ipv4> ipv4 = m_interfaces[i].first;
  ...
  if (router)
    {
      ...
      Ipv4Address routerAddress = ipv4->GetAddress (m_interfaces[i].second, 0).GetLocal ();
...
void Ipv4InterfaceContainer::SetDefaultRoute (uint32_t i, uint32_t router)
{
  Ptr<Ipv4> ipv4 = m_interfaces[i].first;
  Ptr<Ipv4> ipv4Router = m_interfaces[router].first;
  Ipv4Address routerAddress = ipv4Router->GetAddress (m_interfaces[router].second, 1).GetLocal ();
  Ptr<Ipv4StaticRouting> routing = 0;
  Ipv4StaticRoutingHelper routingHelper;
