...
static int event, /* current simulation event            */
...
static real clock, /* current simulation time             */
start, /* simulation interval start time      */
tl; /* last trace message issue time       */

static int l1[nl], l2[nl], /*      facility descriptor,           */
l3[nl]; /*           queue, &                  */
static real /*          event list                 */
l4[nl], /*         element pool                */
l5[nl];

static char name[ns]; /* model, facility, & table name space */
...
void smpl(int m, char *s) {
	...
	rns = stream(rns);
	rns = ++rns > 15 ? 1 : rns; /* set random no. stream */
...
static int save_name(char *s, int m) {
	...
	if (n > m)
		then n = m;
	if (avn + n > ns)
		then error(2, 0); /* namespace exhausted */
	i = avn;
	avn += n + 1;
	strncpy(&name[i], s, n);
	if (n == m)
...
static int get_blk(int n) {
	int i;
	if (blk == 0)
		then error(3, 0); /* block request after schedule */
	i = blk;
	blk += n;
	if (blk >= nl)
		then error(1, 0); /* element pool exhausted */
	return (i);
}
...
static int get_elm() {
	...
	{
		if (avl == 0)
			then error(1, 0); /* empty element list */
...
void schedule(int ev, real te, int tkn) {
	int i;
	if (te < 0.0)
		then error(4, 0); /* negative event time */
	i = get_elm();
	l2[i] = tkn;
	l3[i] = ev;
	l4[i] = 0.0;
...
void cause(int *ev, int *tkn) {
	int i;
	if (evl == 0)
		then error(5, 0); /* empty event list  */
	i = evl;
	*tkn = token = l2[i];
	*ev = event = l3[i];
	clock = l5[i];
...
static int suspend(int tkn) {
	...
	if (succ == 0)
		then error(6, 0); /* no event scheduled for token */
	if (succ == evl)
		then evl = l1[succ]; /* unlink  event */
	else
		l1[pred] = l1[succ]; /* list entry    */
	if (tr)
		then msg(6, -1, "", l3[succ], 0);
	return (succ);
}
...
static void enlist(int *head, int elm) { /* 'head' points to head of queue/event list */
	...
	arg = l5[elm];
	succ = *head;
	while (1) { /* scan for position to insert entry:  event list is order- */
...
int preempt(int f, int tkn, int pri) {
	...
	{ /* facility nonbusy - locate 1st-found nonbusy server     */
		...
		r = 0;
		if (tr)
			then msg(8, tkn, fname(f), 0, 0);
	} else { /* facility busy - find server with lowest-priority user  */
		k = f + 2;
		j = l1[f] + f + 1; /* indices of server elements 1 & n  */
		for (i = f + 2; i <= j; i++)
			if (l2[i] < l2[k])
				then k = i;
		if (pri <= l2[k]) then
		{ /* requesting token's priority is not higher than   */
			...
			r = 1;
			if (tr)
				then msg(7, tkn, fname(f), 1, l3[f]);
		} else { /* preempt user of server k.  suspend event, save   */
...
void release(int f, int tkn) {
	...
	if (j == 0)
		then error(7, 0); /* no server reserved */
	l1[j] = 0;
	l3[j]++;
	l4[j] += clock - l5[j];
	l2[f]--;
	if (tr)
		then msg(9, tkn, fname(f), 0, 0);
	if (l3[f] > 0) then
	{ /* queue not empty:  dequeue request ('k' =  */
		...
		{ /* blocked request:  place request at head of event   */
			...
			evl = k;
			m = 4;
		} else { /* return after preemption:  reserve facility for de- */
...
static void msg(int n, int i, char *s, int q1, int q2) {
	static char *m[14] = { "", "SCHEDULE", "CAUSE", "CANCEL", "   RESCHEDULE",
			"   RESUME", "   SUSPEND", "REQUEST", "PREEMPT", "RELEASE",
			"   QUEUE", "   DEQUEUE", "   RESERVE", "FACILITY" };
	if (clock > tl) /* print time stamp (if time has advanced) */
	then {
		tl = clock;
		fprintf(opf, "  time %-12.3f  ", clock);
	} else
		fprintf(opf, "%21s", m[0]);
	...
	case 5:
	case 6:
		fprintf(opf, " EVENT %d", q1);
		break;
	case 7:
	case 8:
		switch (q1) {
		case 0:
			fprintf(opf, ":  RESERVED");
			break;
		case 1:
			fprintf(opf, ":  QUEUED  (inq = %d)", q2);
			break;
		case 2:
			fprintf(opf, ":  INTERRUPT");
			break;
		default:
			break;
		}
	...
	case 10:
	case 11:
		fprintf(opf, " token %d  (inq = %d)", q1, q2);
		break;
	case 12:
		fprintf(opf, " for token %d", q1);
		break;
	case 13:
		fprintf(opf, ":  f = %d", q1);
		break;
...
/*-----------------------------  PAUSE  ------------------------------*/
void pause() { /* pause execution via 'mtr' call (if active) */
...
void error(int n, char *s) {
	FILE *dest;
	static char *m[8] = { "Simulation Error at Time ", "Empty Element Pool",
			"Empty Name Space", "Facility Defined After Queue/Schedule",
			"Negative Event Time", "Empty Event List",
			"Preempted Token Not in Event List",
			"Release of Idle/Unowned Facility" };
	dest = opf;
	while (1) { /* send messages to both printer and screen */
		fprintf(dest, "\n**** %s%.3f\n", m[0], clock);
		if (n)
			fprintf(dest, "     %s\n", m[n]);
		if (s)
...
static int rept_page(int fnxt) {
	int f, i, n;
	char fn[19];
	static char *s[7] = { "smpl SIMULATION REPORT", " MODEL: ", "TIME: ",
			"INTERVAL: ", "MEAN BUSY     MEAN QUEUE        OPERATION COUNTS",
			" FACILITY          UTIL.    ",
...
/*----------------------------  NEW PAGE  ----------------------------*/
void newpage() { /* set line count to top of page/screen after page change/screen  */
