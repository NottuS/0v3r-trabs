...
__kernel void cl_boarderSolver(__global int *iboard, __global int *oboard, 
	...
	if(lIdx == 1){
		aux = (gIdx + n - 1) % n;
		localBoard[0][0] = iboard[aux * m + n - 2];
		localBoard[0][1] = iboard[aux * m + n - 1];
		localBoard[0][2] = iboard[aux * m];
		localBoard[0][3] = iboard[aux * m + 1];
	}
	if(lIdx == 256 || gIdx + 1 == n){
		aux = (gIdx + 1) % n;
			localBoard[down][0] = iboard[aux * m + n - 2];
		localBoard[down][1] = iboard[aux * m + n - 1];
		localBoard[down][2] = iboard[aux * m];
		localBoard[down][3] = iboard[aux * m + 1];
	}
	barrier(CLK_LOCAL_MEM_FENCE);
	sum = localBoard[up][0] + localBoard[up][1] + localBoard[up][2]
		+ localBoard[lIdx][0] + localBoard[lIdx][2]
...
__kernel void cl_innerGoL(__global int *iboard, __global int *oboard, 
	...
	if(gIdx < colSize){
	    ...
	    center = (up + 1 ) & 3;
	    down = (up + 2) & 3;
	    localBoard[center][lIdx] = iboard[(n - 1) * m + gIdx];
